...
namespace lei3d
{
	...
	void StaticCollider::SetColliderToModel(Model& model, bool addToTriggers)
	{
		std::vector<btTriangleMesh*>& modelMeshes = model.GetCollisionMeshes();
		for (auto triMesh : modelMeshes)
		{
			AddCollisionsFromTriangleMesh(triMesh, m_Entity.m_Transform, addToTriggers);
		}
	}
	 ...
	 * @param transform
	 */
	void StaticCollider::AddCollisionsFromTriangleMesh(btTriangleMesh* triMesh, const Transform& transform, bool addToTriggers)
	{
		...
		if (addToTriggers)
		{
			world.m_triggersWorld->addRigidBody(m_RigidBody);
		} else
		{
